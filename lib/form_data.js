...
function createEntry(name, value /*, filename*/) {
  /*
    ...
    if(value instanceof Buffer) {
      // I am not sure how to conver the blob into a File, but even that
      // this example is following the browser API instead of NodejS
    ...
    if(value instanceof File) {
      // I am not sure how to conver a file into another File, but even that
      // this example is following the browser API instead of NodejS
...
FormData.prototype.set = function(field, value, options) {
  var foundEntryIndex = this._entryList.findIndex(function(entry) {
    return entry.name === field;
  });

  // 4. Otherwise, append entry to the context object’s entry list.
  if (foundEntryIndex === NOT_FOUND) {
    return this.append(field, value, options);
  }
  ...
  // Replace the firt entry
  var newEntry = createEntry(field, value);
  this._entryList[foundEntryIndex] = newEntry;

  // Remove the other entries
  this._entryList = this._entryList.filter(function(entry, index) {
    return index === foundEntryIndex || entry.name !== field;
  });
...
FormData.prototype.append = function(field, value, options) {
  ...
  */
  ...
  var entry = createEntry(field, value);
