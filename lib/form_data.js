 ...
 * @param {*} filename The entry file name when the value is a File.
 */
function FormDataEntry(
  ...
  /*
    ...
    I am not sure how to conver the blob into a File since we do not
    have File or Blob in NodeJS
    ...
    if(value instanceof Blob/Buffer) {
      this.value = new File(value, 'blob');
    }

    Case 2:
    If value is a File object and filename is given, then set value to a
    new File object, representing the same bytes, whose name attribute value
    is filename.

    if(value instanceof File) {
      this.value = new File(value, filename);
    }
  */
  this.value = value;
}

FormDataEntry.prototype.is = function(name) {
...
FormData.prototype.set = function(field, value, options) {
  var found = false;
  var i = 0;

  // 3. If there are any entries in the context object’s entry list whose name
  // is name, then replace the first such entry with entry and remove the
  // others.
  while (i < this._entryList.length) {
    if (this._entryList[i].name === field) {
      if (!found) {
        this._entryList[i] = new FormDataEntry(field, value);
        found = true;
      } else {
        this._entryList.splice(i, 1);
        continue;
      }
    }
    i++;
  ...
  if (!found) {
    this.append(field, value, options);
  }
};

FormData.prototype.append = function(field, value, options) {
  /**
    Since options should be renamed to filename and value could
    be a File or a Blob on the browser as well.

    Maybe we should expect a Buffer since we do not have on NodeJS
  */
...
FormData.prototype.delete = function(field) {
